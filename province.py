...
# db = {"ip": 'localhost', "user": 'root', "pwd": 'account4FTQ', "database": 'tender', "table": 'province'}
db = {"ip": '47.104.188.61', "user": 'root', "pwd": 'Root@123!', "database": 'zb', "table": 'province'}
keys = {}
'''
    ...
    def run(self):
        ...
        while not CRAWL_EXIT:
            try:
                ...
                print(self.threadName + "正在解析%s" % (url))
            except:
                pass
...
class ThreadParse(threading.Thread):
    ...
    def parse(self, html):
        ...
        if data["category"] is not None:
            global count
            count += 1
            print(self.threadName + "正在写入第%d条数据, 采购项目为%s" % (count, data["item"]))
...
def getURL(name):
    ...
    lastId = None
    file = './lmm/province.txt'
    with open(file, 'r') as f:
        lastId = f.readline().strip()

    # 构建url
    url = 'http://www.ccgp-shandong.gov.cn/sdgp2017/site/channelall.jsp?colcode=0301'
...
def getKeywords():
    connection = pymysql.connect(db["ip"], db["user"], db["pwd"], db["database"],
